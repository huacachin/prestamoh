    ...
    public function handle(): int
    {
        ...
        foreach (User::with('roles')->get() as $u) {
            foreach ($u->roles as $r) {
                if (!array_key_exists($r->name, self::MAP)) continue;
        ...
        // Limpiar permisos directos huérfanos: el modelo nuevo asigna por rol.
        $clearedDirect = DB::table('model_has_permissions')->delete();
        if ($clearedDirect > 0) {
            $this->line("  · Borrados $clearedDirect permisos directos legacy (ahora todo via rol).");
        }
