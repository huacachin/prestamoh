...
/**
 * 26/09/2026: en editar ingreso las imágenes se eligen en el formulario y se
 * suben con el MISMO "Guardar cambios" (antes: un campo suelto de una imagen
 * y otro botón "Subir" aparte en la galería de abajo).
 */
class EditarIngresoConImagenesTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        foreach (['caja.editar-historico', 'caja.eliminar', 'caja.ingresos'] as $p) {
            $this->director->givePermissionTo(Permission::findOrCreate($p, 'web'));
        }
        app(PermissionRegistrar::class)->forgetCachedPermissions();
        $this->actingAs($this->director);

        $this->ingreso = Income::create([
            'date' => now()->format('Y-m-d'), 'reason' => 'Ingreso Prueba', 'detail' => 'test',
    ...
    {
        Livewire::test(EditIncome::class, ['id' => $this->ingreso->id])
            ...
            ->call('update')
            ->assertHasNoErrors()
            ->assertRedirect(route('cash.incomes'));

        $this->assertSame('detalle nuevo', $this->ingreso->fresh()->detail);
        $adjuntos = IncomeAttachment::where('income_id', $this->ingreso->id)->get();
    ...
    public function test_sin_imagenes_guarda_igual_que_siempre(): void
    {
        ...
        $this->assertEqualsWithDelta(150.0, (float) $this->ingreso->fresh()->total, 0.001);
        $this->assertSame(0, IncomeAttachment::where('income_id', $this->ingreso->id)->count());
    }
    ...
    public function test_un_archivo_que_no_es_imagen_se_rechaza_y_no_se_guarda_nada(): void
    {
        ...
        $this->assertSame('test', $this->ingreso->fresh()->detail);
        $this->assertSame(0, IncomeAttachment::where('income_id', $this->ingreso->id)->count());
    }

    public function test_el_formulario_tiene_la_zona_de_imagenes_y_un_solo_boton_de_guardar(): void
    {
        $comp = Livewire::test(EditIncome::class, ['id' => $this->ingreso->id]);
    ...
    public function test_la_galeria_embebida_solo_lista_y_no_sube(): void
    {
        IncomeAttachment::create([
            'income_id' => $this->ingreso->id, 'filename' => 'x.jpg', 'original_name' => 'existente.jpg',
            'path' => "incomes/{$this->ingreso->id}/x.jpg", 'mime' => 'image/jpeg', 'size' => 10, 'uploaded_by' => 'director-img',
        ]);

        Livewire::test(IncomeGallery::class, ['id' => $this->ingreso->id, 'embedded' => true])
        ...
        Livewire::test(IncomeGallery::class, ['id' => $this->ingreso->id])
