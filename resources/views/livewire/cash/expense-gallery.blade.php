<div class="{{ $embedded ? '' : 'container-fluid' }}">
    ...
    <div class="row">
        <div class="col-sm-6">
            <h4 class="main-title title-modules">ADJUNTOS DEL EGRESO</h4>
        </div>
        <div class="col-sm-6 mt-sm-2">
            <ul class="breadcrumb breadcrumb-start float-sm-end">
                ...
                <li class="d-flex active">
                    <span class="f-s-14">Adjuntos #{{ $expense->id }}</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    <h6 class="mb-0" style="color:red;">
                        Egreso #{{ $expense->id }} · {{ $expense->reason }}
                    </h6>
                    <small class="text-muted">
                        {{ $expense->date?->format('d/m/Y') }} · S/ {{ number_format($expense->total, 2) }}
                        @if($expense->modo) · <span class="badge bg-secondary">{{ $expense->modo }}</span> @endif
                    </small>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('cash.expenses') }}" class="btn btn-sm btn-secondary">
                        <i class="ti ti-arrow-back"></i> Regresar a egresos
                    </a>
                </div>
            </div>
            @endif
            ...
            {{-- Embebida en editar egreso (26/09) no sube nada: ahí las imágenes
            ...
            @if($attachments->isEmpty())
                <div class="text-center py-5 text-muted">
                    ...
                    <p class="mt-2 mb-0">No hay adjuntos para este egreso.</p>
                    @if($puedeEditar)
                    ...
                    <div class="row g-2">
                        @foreach($attachments as $i => $att)
                            <div class="col-6 col-sm-4 col-md-4 col-lg-4">
                                <div class="border rounded p-1 h-100 d-flex flex-column" style="background:#fafafa;">
                                    <button type="button"
