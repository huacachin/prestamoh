                ...
                <div class="perm-row span-2">
                    ...
                    <div class="perm-col-controls">
                        <div class="perm-chips">
                            @forelse($roles as $r)
                                @php $asignable = in_array($r->name, \Database\Seeders\RoleSetupSeeder::ROLES_ASIGNABLES, true); @endphp
                                <label class="chip-radio" @if(! $asignable) style="opacity:.45; cursor:not-allowed;" title="Rol deshabilitado por ahora" @endif>
                                    <input type="radio" class="form-check-input" name="role_single_add" value="{{ $r->id }}" wire:model="selectedRoleId" @if(! $asignable) disabled @endif>
