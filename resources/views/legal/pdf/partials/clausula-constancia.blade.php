...
<div class="clausula">
    ...
    @if ($vm->destino === 'tercero')
        ...
        <ul class="vinetas">
            <li>INSERTO: EL COMPROBANTE DEL DEPÓSITO/TRANSFERENCIA BANCARIA POR EL IMPORTE TOTAL DE {{ $vm->monto('obligacion') }} EFECTUADO EN EL {{ $banco }}, A LA CUENTA DE {{ mb_strtoupper($vm->tercero['nombre']) }}, POR AUTORIZACIÓN EXPRESA DE {{ $vm->g->deudor() }} {{ $nombres }}, CUYO TENOR GRÁFICO Y LITERAL SE ENCUENTRA EN EL ANEXO 2 DEL PRESENTE CONTRATO.</li>
        </ul>
    @elseif ($vm->destino === 'gerente')
        <ul class="vinetas">
            ...
            <li>INSERTO: EL COMPROBANTE DEL DEPÓSITO/TRANSFERENCIA BANCARIA A FAVOR DEL GERENTE GENERAL DE {{ $vm->g->deudor() }} {{ $nombres }} POR EL IMPORTE TOTAL DE {{ $vm->monto('obligacion') }} EN EL {{ $banco }}, CUYO TENOR GRÁFICO Y LITERAL SE ENCUENTRA EN EL ANEXO 2 DEL PRESENTE CONTRATO.</li>
        </ul>
    @else
        <ul class="vinetas">
            ...
            <li>INSERTO: EL COMPROBANTE DEL DEPÓSITO/TRANSFERENCIA BANCARIA A FAVOR DE {{ $vm->g->deudor() }} {{ $nombres }} POR EL IMPORTE TOTAL DE {{ $vm->monto('obligacion') }} EN EL {{ $banco }}, CUYO TENOR GRÁFICO Y LITERAL SE ENCUENTRA EN EL ANEXO 2 DEL PRESENTE CONTRATO.</li>
        </ul>
    @endif
</div>
